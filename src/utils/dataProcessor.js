...
export const convertPriceToNumber = (priceString) => {
  ...
  const priceStr = String(priceString).replace(/[$,]/g, '');
...
export const processAppsData = (rawData) => {
  ...
  return rawData
    ...
    .map(app => ({
      app: app.App && typeof app.App === 'string' ? app.App.trim() : String(app.App || '').trim(),
      category: app.Category && typeof app.Category === 'string' ? app.Category.trim() : String(app.Category || '').trim(),
      rating: parseFloat(app.Rating) || null,
      reviews: parseInt(app.Reviews) || 0,
      size: app.Size && typeof app.Size === 'string' ? app.Size.trim() : String(app.Size || '').trim(),
      sizeBytes: convertSizeToBytes(app.Size),
      installs: app.Installs && typeof app.Installs === 'string' ? app.Installs.trim() : String(app.Installs || '').trim(),
      ...
      type: app.Type && typeof app.Type === 'string' ? app.Type.trim() : String(app.Type || '').trim(),
      price: app.Price,
      priceNumber: convertPriceToNumber(app.Price),
      contentRating: app['Content Rating'] && typeof app['Content Rating'] === 'string' ? app['Content Rating'].trim() : String(app['Content Rating'] || '').trim(),
      genres: app.Genres && typeof app.Genres === 'string' ? app.Genres.trim() : String(app.Genres || '').trim(),
      lastUpdated: app['Last Updated'] && typeof app['Last Updated'] === 'string' ? app['Last Updated'].trim() : String(app['Last Updated'] || '').trim(),
      currentVersion: (app['Current Ver'] && String(app['Current Ver']) !== 'NaN' && typeof app['Current Ver'] === 'string') ? app['Current Ver'].trim() : (app['Current Ver'] ? String(app['Current Ver']).trim() : null),
      androidVersion: (app['Android Ver'] && String(app['Android Ver']) !== 'NaN' && typeof app['Android Ver'] === 'string') ? app['Android Ver'].trim() : (app['Android Ver'] ? String(app['Android Ver']).trim() : null),
      // Derived fields
...
export const processReviewsData = (rawReviews) => {
  ...
  return rawReviews
    ...
    .map(review => ({
      app: review.App && typeof review.App === 'string' ? review.App.trim() : String(review.App || '').trim(),
      translatedReview: review.Translated_Review && typeof review.Translated_Review === 'string' ? review.Translated_Review.trim() : String(review.Translated_Review || '').trim(),
      sentiment: review.Sentiment && typeof review.Sentiment === 'string' ? review.Sentiment.trim() : String(review.Sentiment || '').trim(),
      sentimentPolarity: parseFloat(review.Sentiment_Polarity) || 0,
      sentimentSubjectivity: parseFloat(review.Sentiment_Subjectivity) || 0,
      // Derived fields
      isPositive: review.Sentiment && String(review.Sentiment).toLowerCase() === 'positive',
      isNegative: review.Sentiment && String(review.Sentiment).toLowerCase() === 'negative',
      isNeutral: review.Sentiment && String(review.Sentiment).toLowerCase() === 'neutral'
    }));
};
